if RUBY_PLATFORM != 'opal'
	...
	describe Volt::HttpRequest do
		...
		it "should report the correct format" do
			env = env_for("http://example.com/test.html", "CONTENT_TYPE" => "text/plain;charset=utf-8")
			request = Volt::HttpRequest.new(env)
			expect(request.format).to eq("html")

			env = env_for("http://example.com/test", "CONTENT_TYPE" => "text/plain;charset=utf-8")
			request = Volt::HttpRequest.new(env)
		...
		it "should return the correct http method" do
			...
			env = env_for("http://example.com/test.html", method: "POST", params: { _method: "put" })
			request = Volt::HttpRequest.new(env)
