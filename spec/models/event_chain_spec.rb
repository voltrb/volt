...
describe EventChain do
  ...
  it "should free events when events are unbound" do
    ...
    expect(a.reactive_manager.event_chain.instance_variable_get('@event_chain')).to eq({})

    listener1 = b.on('changed') { }
    expect(a.reactive_manager.event_chain.instance_variable_get('@event_chain')).to_not eq({})
