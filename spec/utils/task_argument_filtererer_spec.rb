...
if RUBY_PLATFORM != 'opal'
  describe TaskArgumentFilterer do
    ...
    it 'should create and run a new TaskArgumentFilterer when its filter method is called' do
      filtered_args = TaskArgumentFilterer.new([{login: 'jim@jim.com', password: 'some password'}]).run
      expect(filtered_args).to eq([{:login=>"jim@jim.com", :password=>"[FILTERED]"}])
      another_filtered_args = TaskArgumentFilterer.filter([{login: 'jam@jam.com', password: 'some password'}])
      expect(another_filtered_args).to eq([{:login=>"jam@jam.com", :password=>"[FILTERED]"}])
    end
